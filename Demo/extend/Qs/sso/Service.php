...
Class Service {
    ...
    protected $__config = [
        ...
        'Try'  => ['secret'=>'6y4fcghmdt'], // 接入应用设置例子  接收到APP的secret、设置里APP的secret也应该用MD5处理来对比，
        'YiLiao'  => ['secret'=>'6y4fcghmdt'], // 接入应用设置例子  接收到APP的secret、设置里APP的secret也应该用MD5处理来对比，
    ];
    ...
    public function login($appName, $appSecret, $username, $password) {
        ...
        if ( !empty($result) && $this->set(md5($cookieName . $username), $username, $this->__config['COOKIE_TIME']) ) {
            setcookie( $cookieName, md5($cookieName . $username),   time() + $this->__config['COOKIE_TIME'], '/');
            return $cookieName;
        }
        return false;
    }
     ...
     */
    public function logout($appName, $appSecret) {
        if ( !$this->check_app($appName, $appSecret) ) return false;
        $cookieName = $this->get_cookie_name();
        if ( !empty($_COOKIE[$cookieName]) ) {
            $this->handler->rm($_COOKIE[$cookieName]);
        }
